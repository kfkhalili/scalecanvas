...
beforeEach(() => {
  useQuestionStore.setState({ activeQuestion: null, hintIndex: 0 });
});
...
describe("questionStore", () => {
  ...
  it("setInitialQuestion sets activeQuestion and resets hintIndex to 0", () => {
    ...
    expect(useQuestionStore.getState().activeQuestion).toBe(sampleQuestion);
  ...
  it("setInitialQuestion resets hintIndex when question was already set", () => {
    ...
    expect(useQuestionStore.getState().activeQuestion).toBe(other);
    expect(useQuestionStore.getState().hintIndex).toBe(0);
  });
});
